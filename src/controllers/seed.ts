...
export const gigSeed = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
        ...
        await publishDirectMessage(gigChannel, 'gigconnect-gig', 'get-sellers', JSON.stringify({ type: 'getSellers', count: parseInt(count) }), 'Message sent to users service to get sellers.');
